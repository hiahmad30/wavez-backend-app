...
export default class RentalsModel implements IModel {
    ...
    public static getInstance(): IModel {
        if (!this._instance) this._instance = new RentalsModel();
    ...
    public getModel(): Model<Document> {
        if (!this._model) {
            this._model = mongoose.model('Rentals', this._getSchema(), 'rentals');
        }
        return this._model;
    }
}
